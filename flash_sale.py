...
# Read the .xls file
df = pd.read_excel('D:/Downloads/fs.xls')

for index, row in df.iterrows():
    serial = index.__hash__() + 2
    price = math.floor(float(row['Recommended Price'].replace('<=', '')))
    seller_sku = row['Seller SKU'].lower()
    product_tag, variation = seller_sku.split('-')
    ...
    cursor.execute('SELECT price, max_price FROM pricing WHERE product_tag = ?', (identical_product_tag,))
    try:
    ...
    # Set price
    if is_multi():
        df.loc[index, 'Campaign Price'] = price
        cursor.execute('UPDATE campaign_data SET campaign_price = ? WHERE serial = ?',
                       (price, serial,))
    else:
        df.loc[index, 'Campaign Price'] = min(max_price, price)
        cursor.execute('UPDATE campaign_data SET campaign_price = ? WHERE serial = ?',
...
for product_tag in cursor.fetchall():
    ...
    if not cursor.fetchall():
        cursor.execute('SELECT recommended_price FROM campaign_data WHERE product_tag = ?', (product_tag[0],))
        print(product_tag, 'Permanently removed at', cursor.fetchall())

print(f' Total row: {df.shape[0]} '.center(40, '━'))
df.to_excel('D:/Downloads/fs.xlsx', index=False)
