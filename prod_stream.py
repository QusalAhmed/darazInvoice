...
def is_duplicate(image_hash, cutoff=50):
...
def square_image(image_src):
    ...
    if width == height:
        return get_image_url(image_src)
...
# Delete all the files in the directory: New Product
...
if os.path.exists(os.path.join(directory, 'product_info.xlsx')):
    wb = load_workbook(os.path.join(directory, 'product_info.xlsx'))
else:
    wb = Workbook()
    ws = wb.active
    ws.title = 'Video URL'
    ...
    for col in ['A', 'B']:
        ...
        wb['Attributes'][col + '1'].alignment = Alignment(horizontal='center', vertical='center',
                                                          wrap_text=True)
        wb['Attributes'][col + '1'].fill = PatternFill(start_color="00FFFF", end_color="00FFFF", fill_type="solid")
        # Set font size
...
# Now it's time to upload the product
worksheet = load_workbook(os.path.join(directory, 'product_info.xlsx'))
...
if product_name_bn is None:
    ...
    product_name_bn = model.generate_content(
        [f":Translate {product_name_eng} to Bengali", "Only give translated text, don't add extra information"]).text
...
for shop_name, access_token in cursor.fetchall():
    ...
    for serial in range(1, 9):
        ...
        if image_path:
            Et.SubElement(main_images, "Image").text = get_image_url(image_path[0])
        else:
            break
    ...
    if image_path:
        root.find('Product').find('Attributes').find('promotion_whitebkg_image').text = square_image(image_path)
    else:
        root.find('Product').find('Attributes').find('promotion_whitebkg_image').text = square_image('1')
    # SKU image
    for index, sku_element in enumerate(root.find('Product').find('Skus').findall('Sku')):
        sku_element.find('Images').find('Image').text = get_image_url(sku_image_list[int(index)])
